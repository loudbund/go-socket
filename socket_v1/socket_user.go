...
type SocketUser struct {
	...
	Addr     string
	Server   *Server
	C        chan DataUnitSocket
}

// 创建一个用户的API
func NewUser(conn net.Conn, server *Server) *SocketUser {
	user := &SocketUser{
...
// 启动运行
func (Me *SocketUser) goListenClientMsg() {
	// 监听用户是否活跃的channel
	...
	go func() {
		if err := Me.getSocketMsg(Me.Conn, func(msg *DataUnitSocket) bool {
			...
			if msg.CType == 7 {
				...
				_ = Me.SendSocketMsg(Me.Conn, DataUnitSocket{0, 8, []byte("hello test msg from server")})
			}
...
// 等待心跳
func (Me *SocketUser) waitHeartBeet(isLive chan bool) {
	// 阻塞住
	for {
		select {
		case msg := <-Me.C:
...
func (Me *SocketUser) Online() {
	...
	// 2、事件发给server
	Me.Server.ChanHookEvent <- &HookEvent{"online", Me, DataUnitSocket{}}
}

// 用户的下线业务
func (Me *SocketUser) Offline() {
	...
	// 2、事件发给server
	Me.Server.ChanHookEvent <- &HookEvent{"offline", Me, DataUnitSocket{}}
}
